...
Rx<UserProfileDataModel>userProfileDataModel=Rx(UserProfileDataModel());
  ...
  verifyOtp(){
    ...
    ApiRequest apiRequest=ApiRequest(url:ApiPath.emailOtpVerifyUrl,body: body);
    apiRequest.postRequest(isLoadingScreen: false).then((res){
      if(res!.statusCode==200){
      ...
      if(peapleApiData !=null || peapleApiData !=''){
        ...
        dod = '${peapleApiData['birthdays'][0]['date']['year']}-${peapleApiData['birthdays'][0]['date']['month']}-${peapleApiData['birthdays'][0]['date']['day']}';
          ...
          if (userCredential.value!.user!.photoURL != null && userCredential.value!.user!.photoURL!.isNotEmpty) {
            ...
            final photoURL = await http.get(Uri.parse(userCredential.value!.user!.photoURL!));
...
// Prepare and send request
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(userProfileDataModel.value.message ?? 'Something went wrong')),
      );
    }
  } catch (e) {
    Get.back();
  ...
  Future<void> signOutFromGoogle() async {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (isGoogle) {
          await googleSignIn.signOut(); // or disconnect() if you want to revoke access
        }
