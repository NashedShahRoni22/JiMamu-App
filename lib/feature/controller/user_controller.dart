...
class UserController extends GetxController{
  ...
  final Box<UserProfileDataModel> userBox = Hive.box<UserProfileDataModel>(LocalString.USER_PROFILE_BOX);
  final Box<UpdateRiderDataModel> riderBox = Hive.box<UpdateRiderDataModel>(LocalString.RIDER_PROFILE_BOX);

  UserProfileDataModel get userProfile=>userBox.get('user')??UserProfileDataModel();
  ...
  updateUserProfile(BuildContext context) async {
    ...
    try {
      ...
      var request = http.MultipartRequest('POST', Uri.parse('https://jimamu.sneakpeekbd.com/api/v1/rider/profile/update'));
      ...
      if(otherFile !=null){
        request.files.add(await http.MultipartFile.fromPath('document[]', otherFile!.path));
      }

      if(fontFile !=null){
        request.files.add(await http.MultipartFile.fromPath('document[]', fontFile!.path));
      }

      if(backSideFile !=null){
        request.files.add(await http.MultipartFile.fromPath('document[]', backSideFile!.path));
      }
      ...
      if (response.statusCode == 200) {
        ...
        // print(await response.stream.bytesToString());
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(updateRiderDataModel.value.message ?? "Update success")),
          );
          Get.back();
      }
      else {
  ...
  updateUserData() {
    docTypeController.text = updateRiderDataModel.value.data?.riderDocument?.first.documentType ?? "";
    docNumberController.text =updateRiderDataModel.value.data?.riderDocument?.first.documentNumber ?? "";
    phoneController.text = updateRiderDataModel.value.data?.riderDocument?.first.documentType??"";
    cardNumberController.text = updateRiderDataModel.value.data?.riderBankInformation?.first.accountNumber??"";
    cvcNumberController.text = updateRiderDataModel.value.data?.riderBankInformation?.first.cvcCode??"";
    expireDateController.text = updateRiderDataModel.value.data?.riderBankInformation?.first.expiryDate??"";
  }
